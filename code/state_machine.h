...
/* The elevator hardware is initialized first (see elev.h). If this is done successfully,
   the elevator is driven down to the nearest floor if it did not start at a floor already.
...
/* Given the current elevator state and the overall situation, the conditions for
   ...
   are executed. The possible state transitions are listed after the current state:
   From IDLE_AT_FLOOR:
   ...
   From DRIVE:
     to EMERGENCY_AT_FLOOR: If the stop button is pressed and the elevator is at a floor.
     to EMERGENCY_BETWEEN_FLOORS: If the stop button is pressed and the elevator
                                  is located between 2 floors.
     to OPEN_DOOR: If the elevator reaches a new floor and there is a cab order to
                   that floor or a hall order that should be served.
     Note that each time the elevator reaches a new floor the corresponding floor
     indicator light is updated.
   From OPEN_DOOR:
     to EMERGENCY_AT_FLOOR: If the stop button is pressed.
     to DRIVE: If there is an order at a different floor and the door has been open
               long enough.
     to IDLE_AT_FLOOR: If there are no more orders and the door has been open long enough.
