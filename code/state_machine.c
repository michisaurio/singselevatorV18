...
/* elevator_direction represents in a broader context the direction the elevator
   ...
   of elevator_direction is not necessarily DIRN_STOP.  */
static elev_motor_direction_t elevator_direction;
static int last_floor;
/* move_after_door_closes takes the values 1 or 0 depending on whether it has
   been determined that the elevator is going to move after the door closes.  */
  ...
  switch (elevator_state)
    {
        ...
        if (sensor_signal >= 0 && last_floor != sensor_signal)
          {
            ...
            /* Checks if there is a cab order or a hall order in the same
               direction as the elevator direction. Else, checks if there are no
               further orders to or at floors further in the elevator direction
               and there is a hall order in the direction opposite to the
               elevator direction.  */
      ...
      case OPEN_DOOR:
        ...
        /* Pressing the cab order button for the current floor only restarts
           the door timer.  */
        if (get_order_status(BUTTON_COMMAND, last_floor))
          {
            clear_order_status(BUTTON_COMMAND, last_floor);
        ...
        /* If the elevator direction after the door closes has been determined,
           pressing the hall order button in the same direction only restarts
           the door timer. */
        if (move_after_door_closes)
          {
            if (elevator_direction == DIRN_UP
        ...
        /* If the door timers elapsed time is over the time threshold,
           the elevator state changes to DRIVE or to IDLE_AT_FLOOR. */
        update_door_timer();
      ...
      /* Checks if there is an order somewhere. The location of the elevator is
         determined using elevator_direction and last_floor. If the elevator
         state is set to drive and the elevator direction is changed, the value
         of last_floor is modified in order to give the same location as before.
