...
determine_next_state()
{
  ...
  case DRIVE:
    ...
    if (sensor_signal >= 0) {	// i.e. a floor is reached
      ...
      /* Checks if there is a cab order or a hall order in the same direction
         as the elevator direction. Else, checks if there are no further orders
         to or at floors further in the elevator direction and there is a hall
         order in the direction opposite to the elevator direction.  */
  ...
  case OPEN_DOOR:
    ...
    /* The new elevator direction after the door closes is set to DIRN_UP
       if there are any orders at or to floors upstairs and if the elevator direction
       was DIRN_UP or DIRN_STOP. This will also happen if the elevator direction
       was DIRN_DOWN and there only are orders at or to floors upstairs.
    ...
    /* If the door timers elapsed time is over the time threshold, the elevator
       state changes to DRIVE or to IDLE_AT_FLOOR. */
    update_door_timer();
